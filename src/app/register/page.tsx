...
import { FileImage, Image, Mail, User, Lock, CheckCircle2, Circle, EyeOff, Eye } from 'lucide-react';
...
export default function RegisterPage() {
 ...
 const [showPassword, setShowPassword] = useState(false);
  ...
  const strengthScore = [hasUppercase, hasLowercase, isLongEnough].filter(Boolean).length;
  ...
  const getStrengthMeta = () => {
    if (formData.password.length === 0) return { label: 'None', color: 'bg-slate-100' };
    ...
    <div className="w-full min-h-screen flex flex-col md:flex-row bg-white text-[#1e293b] max-w-350 mx-auto py-5 md:py-10 lg:py-15 px-6 md:px-12 lg:px-18 xl:px-24">
      ...
      <div className="hidden md:flex md:w-1/2 lg:w-[45%] bg-linear-to-b from-[#3b82f6] to-[#1d4ed8] text-white p-8 lg:p-12 flex-col justify-between items-center relative overflow-hidden">
        ...
        <div className="text-center max-w-md my-auto px-4">
          ...
          <p className="text-blue-100 text-base lg:text-lg leading-relaxed font-light">
            Join thousands of patients and practitioners in a modern medical ecosystem designed for clarity, speed, and trust.
          </p>
        </div>
          ...
          <div className="w-full h-full opacity-80 flex flex-col justify-between text-[10px] text-slate-400 font-mono">
            ...
            <div className="grid grid-cols-3 gap-2 my-2">
              <div className="bg-slate-900 p-2 rounded border border-slate-800 text-center">
                <span className="block text-white font-bold text-xs">0</span> Slots
              </div>
              <div className="bg-slate-900 p-2 rounded border border-slate-800 text-center">
                <span className="block text-white font-bold text-xs">0</span> Active
              </div>
              <div className="bg-slate-900 p-2 rounded border border-blue-500/40 text-center">
                <span className="block text-cyan-400 font-bold text-xs">+</span> New
              </div>
            </div>
      ...
      <div className="w-full md:w-1/2 lg:w-[55%] border border-blue-500 bg-white px-6 py-12 sm:px-12 lg:px-20 flex flex-col justify-center items-center">
        <div className="w-full max-w-md">
          ...
          {/* Header Text */}
          <h2 className="text-3xl font-bold tracking-tight text-slate-900 mb-1">Create Account</h2>
          <p className="text-sm text-slate-500 mb-8">Start your journey to better health management today.</p>
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            {/* Input: Photo URL */}
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Photo URL</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-slate-400">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className={`w-full pl-10 pr-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-sm transition-all placeholder:text-slate-400 ${
                    passwordTouched && strengthScore < 3 ? 'border-rose-300 focus:border-rose-500 ring-rose-500/5' : 'border-slate-200 focus:border-blue-500'
                  }`}
                />
                <button
                  type="button"
              ...
              <div className="mt-2.5">
                <div className="grid grid-cols-3 gap-1.5">
                  <div className={`h-1 rounded-full transition-colors duration-300 ${strengthScore >= 1 ? strengthMeta.color : 'bg-slate-100'}`}></div>
                  <div className={`h-1 rounded-full transition-colors duration-300 ${strengthScore >= 2 ? strengthMeta.color : 'bg-slate-100'}`}></div>
                  <div className={`h-1 rounded-full transition-colors duration-300 ${strengthScore === 3 ? strengthMeta.color : 'bg-slate-100'}`}></div>
                </div>
                <div className="flex items-center gap-1 mt-1 text-[11px] text-slate-500">
                  <span>Password strength: <strong className="font-semibold text-slate-700">{strengthMeta.label}</strong></span>
                </div>
              </div>

              {/* Requirement Rule Checks Checklist */}
              <ul className="mt-3 space-y-1.5 text-xs border-t border-slate-100 pt-2.5">
                <li className={`flex items-center gap-2 transition-colors duration-200 ${isLongEnough ? 'text-emerald-600 font-medium' : 'text-slate-400'}`}>
                  {isLongEnough ? <CheckCircle2 size={14} className="text-emerald-500" /> : <Circle size={14} className="text-slate-300" />}
                  Minimum length of 6 characters
                </li>
                <li className={`flex items-center gap-2 transition-colors duration-200 ${hasUppercase ? 'text-emerald-600 font-medium' : 'text-slate-400'}`}>
                  {hasUppercase ? <CheckCircle2 size={14} className="text-emerald-500" /> : <Circle size={14} className="text-slate-300" />}
                  At least 1 uppercase letter (A-Z)
                </li>
                <li className={`flex items-center gap-2 transition-colors duration-200 ${hasLowercase ? 'text-emerald-600 font-medium' : 'text-slate-400'}`}>
                  {hasLowercase ? <CheckCircle2 size={14} className="text-emerald-500" /> : <Circle size={14} className="text-slate-300" />}
            ...
            {/* Submit Button */}
            <button type="submit" className="w-full bg-gradient-to-r from-[#3ba2f6] to-[#3b82f6] hover:from-[#2563eb] hover:to-[#3b82f6] text-white font-medium py-2.5 px-4 rounded-lg flex items-center justify-center gap-1.5 transition-all shadow-sm active:scale-[0.99]">
              <span>Register</span>
          ...
          <div className="relative my-6 flex items-center justify-center">
            ...
            <span className="relative bg-white px-3 text-xs uppercase tracking-wider text-slate-400 font-medium">Or continue with</span>
          </div>

          {/* Google Social OAuth Button */}
          <button type="button" className="w-full border border-slate-200 hover:bg-slate-50 text-slate-700 font-medium py-2.5 px-4 rounded-lg flex items-center justify-center gap-2.5 transition-all text-sm shadow-sm">
          ...
          <p className="text-[11px] text-center text-slate-400 mt-10 leading-relaxed px-4">
            By registering, you agree to DocAppoint&#39;s <a href="#terms" className="underline hover:text-slate-600">Terms of Service</a> and <a href="#privacy" className="underline hover:text-slate-600">Privacy Policy</a>.
          </p>
